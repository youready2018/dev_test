...
export default function CampusRecruitPage() {
  const navigate = useNavigate();
  const [jobs, setJobs] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getJobs({ category: 'campus', page: 1, page_size: 50 }).then((res: any) => {
      setJobs(res.items || []);
  ...
  return (
    <div>
      <div className="hero-banner"><div className="hero-bg" /><div className="hero-overlay" />
        <div className="hero-content"><h1>校园招聘</h1><p>青春有梦，TP同行</p></div></div>
      <div className="page-section"><div className="container">
        {loading ? <div style={{ textAlign: 'center', padding: 40 }}>加载中...</div> : (
                    ...
                    <div key={job.id} className="job-card" onClick={() => navigate(`/jobs/detail/${job.id}`)}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                        <div>
                          ...
                          <p style={{ fontSize: 13, color: 'var(--text-light)', marginTop: 4 }}>{job.location} · {job.salary_range || '薪资面议'}</p>
                        </div>
          ...
          }) : <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-light)' }}>暂无校招职位</div>
        )}
      </div></div>
    </div>
  );
